...
public class Scr_Enemigo : MonoBehaviour
{
    ...
    public void realizardaño(float daño, string efecto, string efecto2) 
    {
        // Lógica de daño
        Controlador.GetComponent<Scr_ControladorBatalla>().PuntosActualesHabilidad +=
            GameObject.Find("Singleton").GetComponent<Scr_DatosArmas>().TodasLasArmas[Controlador.GetComponent<Scr_ControladorUIBatalla>().ArmaActual].PuntosXGolpe;
    ...
    public void checarEfecto(string efecto, float porefecto)
    {
        ...
        switch (efecto)
        {
            ...
            case "Empujar":
                ...
                StartCoroutine(EstadoEmpujado(70f, porefecto)); // dirección y fuerza
                break;
    ...
    IEnumerator EstadoEmpujado(float fuerza, float por)
    {
        ...
        if (estaEmpujado) yield break;  // Evitar empujones simultáneos
        estaEmpujado = true;

        NavMeshAgent agente = GetComponent<NavMeshAgent>();
        Rigidbody rb = GetComponent<Rigidbody>();

        // 1. Desactivar NavMeshAgent para controlar con físicas
        if (agente != null)
            agente.enabled = false;

        // 2. Activar físicas reales (no kinematic)
        rb.isKinematic = false;
        rb.useGravity = true;

        // 3. Dirección horizontal del empujón (hacia atrás)
        Vector3 direccionEmpuje = -transform.forward;
        direccionEmpuje.y = 0;
        direccionEmpuje.Normalize();

        // 4. Limpiar velocidad previa para que el empujón sea limpio
        rb.velocity = Vector3.zero;

        // 5. Aplicar la fuerza de empuje en modo Impulse
        rb.AddForce(direccionEmpuje * (fuerza * por), ForceMode.Impulse);

        // 6. Tiempo de empuje (ajustar según tu preferencia)
